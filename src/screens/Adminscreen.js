    ...
    async function fetchData() {
      try {
        const data = await (
          ...
          data.sort((a, b) => new Date(b.createdAt) - new Date(a.updatedAt));
          setBookings(data);
        setLoading(false);
      } catch (error) {
        console.log(error);
            ...
            bookings.map((booking) => {
              return (
                <tr key={booking._id}>
                  ...
                  <td data-label="TotalAmount : ">{booking.totalAmount}</td>
                  <td data-label="TotalDays : ">{booking.totalDays}</td>
...
export function AddingNew() {
  ...
  return (
    <div className="tbl" style={{  display: 'flex', flexDirection: 'column', alignItems: 'center' }}>

      {/* Rooms Section */}
      <div style={{borderRadius: '5px', margin: '10px', padding: '10px',backgroundColor:'#c9d8e2' }}>
    <div className="col-md-3 admin-add-new">
        <h1 className="w">Rooms</h1>
        <div style={{borderRadius: '5px',width: '380%', margin: '10px', padding: '10px',backgroundColor:'#e7f5ff' }}>
        <input
          type="text"
